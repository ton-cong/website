...
const AdminCategoryList = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (imageFile) {
                setUploading(true);
                const res = await productApi.uploadEditorImage(imageFile);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    onClick={() => { setShowModal(true); setEditCategory(null); setForm({ name: '', description: '', imageUrl: '' }); setImagePreview(null); setImageFile(null); }}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Ảnh danh mục</label>
                                {imagePreview && (
                                    <img
                                        src={imagePreview}
                                        alt="preview"
                                ...
                                <label className="flex items-center gap-2 cursor-pointer px-4 py-2 border-2 border-dashed border-slate-300 rounded-lg hover:border-indigo-400 transition-colors w-full justify-center text-slate-500 hover:text-indigo-600">
                                    <PhotoIcon className="h-5 w-5" />
                                    <span className="text-sm">{imagePreview ? 'Đổi ảnh' : 'Chọn ảnh'}</span>
                            ...
                            <div className="flex gap-3 justify-end pt-2">
                                ...
                                >
                                    {uploading ? 'Đang upload...' : editCategory ? 'Cập nhật' : 'Tạo mới'}
                                </button>
                            </div>
                        </form>
                    </div>
